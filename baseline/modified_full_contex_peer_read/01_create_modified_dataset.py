...
data_merged_big = data_merged.groupby("labels").count().sort_values(by="full", ascending=False)

#only include citations, which are cited more than 100 times
big_label_list = data_merged_big.loc[data_merged_big["full"]>=6].reset_index()["labels"].to_list()
...
data_merged_small['labels'] = le.fit_transform(data_merged_small["target_id"].values)

#pickle the new dataframe
data_merged_small.to_pickle("data_merged_small_6.pkl")
